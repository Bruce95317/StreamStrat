...
stock_names = [result.text for result in res_html.find_all('td', {'class': "old_data-col0"})[2:32]]
company_names = [result.text for result in res_html.find_all('td', {'class': "old_data-col1"})[2:32]]

stock_dict = dict()
for stock_name,company_name in zip(stock_names ,company_names):
    stock_dict[stock_name] = company_name

## getting old_data from the last 5 years (2017-2021)
